...
public class HistoricNursingDataController extends CommonControllerConstants {

    @Resource(name = "giveHistoricDao")
    private GiveHistoricDao historicDao;


    @RequestMapping("/givesHistoricData.htm")
    public ModelAndView givesHistoricData(String filter, String dir, String sort, Integer limit, Integer start) throws UnsupportedEncodingException {
        ...
        result.setTotal(historicDao.loadGiveHistoricSizeAll(request).intValue());
        List<GivesHistoric> aux = historicDao.loadGiveHistoricAll(request);
        result.setData(aux);
