...
@Role({"SYSTEM_ADMINISTRATOR",    
    "NURSE_PHARMACY",
    "PHARMACIST"})
public class HistoricPharmacyDataByStaffController extends CommonControllerConstants {

    @Resource(name = "prepareHistoricDao")
    private PrepareHistoricDao historicDao;

    @RequestMapping("/prepareHistoricDataByStaff.htm")
    public ModelAndView givesHistoricDataByStaff(HttpServletRequest request,String filter, String dir, String sort, Integer limit, Integer start) throws UnsupportedEncodingException {
        ...
        result.setTotal(historicDao.loadPrepareHistoricSizeAll(filterRequest).intValue());
        List<PrepareHistoric> aux = historicDao.loadPrepareHistoricAll(filterRequest);
        result.setData(aux);
