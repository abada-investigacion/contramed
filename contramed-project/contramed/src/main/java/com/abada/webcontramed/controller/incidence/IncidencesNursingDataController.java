...
public class IncidencesNursingDataController extends CommonControllerConstants {

    @Resource(name = "incidenceDao")
    private IncidenceDao incidenceDao;

    @RequestMapping("/givesIncidencesData.htm")
    public ModelAndView givesIncidencesData(String filter, String dir, String sort, Integer limit, Integer start) throws UnsupportedEncodingException {
        ...
        result.setTotal(incidenceDao.loadGivesIncidencesSizeAll(request).intValue());
        List<GivesIncidence> aux = incidenceDao.loadGivesIncidencesAll(request);
        result.setData(aux);
