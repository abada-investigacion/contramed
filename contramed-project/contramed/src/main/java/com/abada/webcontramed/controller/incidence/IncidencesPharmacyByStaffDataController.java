...
@Role({"SYSTEM_ADMINISTRATOR",    
    "NURSE_PHARMACY",
    "PHARMACIST"})
public class IncidencesPharmacyByStaffDataController extends CommonControllerConstants {

    @Resource(name = "incidenceDao")
    private IncidenceDao incidenceDao;

    @RequestMapping("/prepareIncidencesDataByStaff.htm")
    public ModelAndView prepareIncidencesDataByStaff(HttpServletRequest request,String filter, String dir, String sort, Integer limit, Integer start) throws UnsupportedEncodingException {
        ...
        result.setTotal(incidenceDao.loadPrepareIncidencesSizeAll(filterRequest).intValue());
        List<PrepareIncidence> aux = incidenceDao.loadPrepareIncidencesAll(filterRequest);
        result.setData(aux);
