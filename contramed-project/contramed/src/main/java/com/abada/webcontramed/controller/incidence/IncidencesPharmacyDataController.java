...
@Role({"SYSTEM_ADMINISTRATOR",    
    "PHARMACIST"})
public class IncidencesPharmacyDataController extends CommonControllerConstants {

    @Resource(name = "incidenceDao")
    private IncidenceDao incidenceDao;

    @RequestMapping("/prepareIncidencesData.htm")
    public ModelAndView prepareIncidencesData(String filter, String dir, String sort, Integer limit, Integer start) throws UnsupportedEncodingException {
        ...
        result.setTotal(incidenceDao.loadPrepareIncidencesSizeAll(request).intValue());
        List<PrepareIncidence> aux = incidenceDao.loadPrepareIncidencesAll(request);
        result.setData(aux);
