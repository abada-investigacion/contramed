        ...
        items: [new Ext.contramedadmin.insertUpdateDeleteToolbar({
                listeners: {
                    submitInsert: function() {
                        handleFormulario('Inserta', grid, 'cama/recurso', 'insertRecursoTag.htm', grid.sm);
                    },
                    submitUpdate: function() {
                        if (grid.sm.hasSelection()) {
                            if (grid.sm.getCount() == 1) {
                                handleFormulario('Modifica', grid, 'cama/recurso', 'updateRecursoTag.htm', grid.sm);
                            } else {
                                Ext.Msg.alert('', 'Seleccione una &uacute;nica Cama');
                            }
                        } else
                            Ext.Msg.alert('', 'Seleccione un Tag de cama/recurso');
                    },
                    submitDelete: function() {
                        if (grid.sm.hasSelection()) {
                            handledelete(grid, 'cama/recurso', 'id', '', '', 'deleteRecursoTag.htm');
                        } else
                            Ext.Msg.alert('', 'Seleccione Tag de cama/recurso');
                    }
                }
...
function handleFormulario(opt, grid, title, url, selecion) {
    ...
    var buttontag = new Ext.contramed.BarcodeButton({
        height: 30,
