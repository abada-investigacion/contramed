 ...
 */
@Repository("doseDao")
public class DoseDaoImpl extends JpaDaoSupport implements DoseDao {
    ...
    public Long loadSizeAll(GridRequest_Extjs_v3 filters) {
        List<Long> result = this.find("select count(*) from Dose d" + filters.getQL("d", true), filters.getParamsValues());
        return result.get(0);
    }
    ...
    public List<Dose> loadAll(GridRequest_Extjs_v3 filters) {
        List<Dose> dose = this.find("from Dose d" + filters.getQL("d", true), filters.getParamsValues(), filters.getStart(), filters.getLimit());
        return dose;
    }
    ...
    public void updateDose(Long iddose, String codigo, String batch, Date expiration_date, BigDecimal give_amount, Integer idmeasure_unit) throws Exception {
        ...
        Dose dose = (Dose) this.getJpaTemplate().find(Dose.class, new Long(iddose));
