 ...
 */
package com.abada.contramed.persistence.dao.managerthreatment.impl;
...
import com.abada.concurrency.monitor.BlockMonitor;
...
import com.abada.contramed.persistence.dao.measureunitconversion.MeasureUnitConversionDao;
...
import com.abada.contramed.persistence.entity.enums.TypePrepareHistoric;
import com.abada.contramed.properties.ContramedProperties;
import com.abada.springframework.orm.jpa.support.JpaDaoSupport;
import com.abada.webcontramed.entities.PrescriptionType;
import com.abada.webcontramed.entities.ThreatmentInfo;
import com.abada.webcontramed.entities.TreatmentActionMode;
import com.abada.webcontramed.entities.TypeThreatmentInfoStatus;
import com.abada.webcontramed.exception.AllergyException;
import com.abada.webcontramed.exception.ExpiredDoseException;
import com.abada.webcontramed.exception.GenericWebContramedException;
import com.abada.webcontramed.exception.ImposibleRemoveThreatmentException;
import com.abada.webcontramed.exception.NoCorrectThreatmentException;
import com.abada.webcontramed.exception.NoExistDoseException;
import com.abada.webcontramed.exception.NoThreatmentException;
import com.abada.webcontramed.exception.RangeTimeOutException;
import com.abada.webcontramed.exception.WebContramedException;
import com.abada.webcontramed.utils.Utils;
...
public class ManagerThreatmentDaoImpl extends JpaDaoSupport implements PrepareThreatmentDao, GiveThreatmentDao {
    ...
    private ContramedProperties contramedProperties;
...
// <editor-fold defaultstate="collapsed" desc="Administrar y preparar tratamientos">
    ...
    private void createPrepareHistoric(OrderTimingInfo info, TypePrepareHistoric typePrepareHistoric, Dose dose, Staff staff, String... observations) throws RangeTimeOutException {
        ...
        historic.setOrderTimingIdorderTiming(this.getJpaTemplate().find(OrderTiming.class, info.getIdOrderTiming()));
        historic.setType(typePrepareHistoric);
        historic.setStaffIdstaff(staff);
        historic.setObservation(Utils.append(observations));
        this.getJpaTemplate().persist(historic);
    }
    ...
    private void createGiveHistoric(OrderTimingInfo info, Dose dose, Staff staff, TypeGivesHistoric typeGivesHistoric, Patient patient, TypeGetPatient typeGetPatient, String... observations) throws RangeTimeOutException {
        ...
        Patient pat = this.getJpaTemplate().find(Patient.class, patient.getId());
    ...
    private boolean isLastGivesHistoricTypeOf(Long idOrderTiming, Date giveDate, TypeGivesHistoric type) {
        List<GivesHistoric> result = this.getJpaTemplate().find("from GivesHistoric gh where gh.orderTimingIdorderTiming.idorderTiming = ? and gh.orderTimingDate = ? order by gh.eventDate desc", idOrderTiming, giveDate);//threatmentInfo.getIdOrderTiming(), threatmentInfo.getGiveTime());
