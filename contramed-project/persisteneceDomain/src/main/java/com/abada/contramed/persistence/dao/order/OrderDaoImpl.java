...
import com.abada.contramed.persistence.dao.table0119.Table0119Dao;
 ...
 */
@Repository("orderDao")
public class OrderDaoImpl extends JpaDaoSupport implements OrderDao {
    ...
    private void save(Order1 order, Patient patient) throws HL7Exception {
        ...
        if (patients != null && patients.size() == 1) {
            ...
            if (ltable0119 != null && ltable0119.size() == 1) {
                ...
                this.getJpaTemplate().persist(order);

                if (order.getOrderMedication() != null) {
                    order.getOrderMedication().setOrderIdorder(order.getIdorder());
                    this.getJpaTemplate().persist(order.getOrderMedication());
                }
    ...
    private void update(Order1 order, Patient patient, Order1 orderoriginalbd) throws HL7Exception {
        ...
        if (order.getOrderMedication() != null) {
            ...
            this.getJpaTemplate().merge(order.getOrderMedication());
    ...
    public List<Order1> findAll() {
        return this.getJpaTemplate().findByNamedQuery("Order1.findAll");
    }
    ...
    public List<Order1> findByOrderId(String orderId, String groupId) {
        return this.getJpaTemplate().find("SELECT o FROM Order1 o WHERE o.orderId = ? and o.orderGroupId=?", orderId, groupId);
    }
}
