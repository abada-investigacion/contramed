 ...
 */
package com.abada.contramed.persistence.dao.patient;
...
import com.abada.contramed.persistence.dao.city.CityDao;
...
import com.abada.contramed.persistence.dao.table0001.Table0001Dao;
import com.abada.contramed.persistence.dao.table0002.Table0002Dao;
...
public class PatientDaoImpl extends JpaDaoSupport implements PatientDao {
    ...
    public void save(Patient patient) throws HL7Exception {
        ...
        this.getJpaTemplate().persist(patient);
    }
            ...
            if (oldPatient.getOrder1List() != null) {
                for (Order1 order : oldPatient.getOrder1List()) {
                    if (order.getOrderMedication() != null) {
                        this.getJpaTemplate().remove(order.getOrderMedication());
                    }
                }
            }
            this.getJpaTemplate().remove(oldPatient);
        } else {
    ...
    public void deleteAll() {
        List<Patient> lpatient = this.getJpaTemplate().findByNamedQuery("Patient.findAll");
    ...
    public void updatebed(Patient patient) throws HL7Exception {
        ...
        if (patients != null && patients.size() == 1) {
            rs = this.getJpaTemplate().find("select r from Recurso r where r.nr='" + patient.getBed().getNr() + "'");
            ...
            this.getJpaTemplate().merge(rs.get(0));
    ...
    private Patient getPatientInBed(String bedId) {
        List aux = this.getJpaTemplate().find("from Patient p where p.bed in (select r.idRecurso from Recurso r where r.nr=?)", bedId);
    ...
    public Patient getPatientByBedTag(String tag) {
        List aux = this.getJpaTemplate().find("from Patient p where p.bed = (select r.recurso from RecursoTag r where r.tag=?)", tag);
        if (aux != null && aux.size() == 1) {
            return (Patient) aux.get(0);
        }
        return null;
    ...
    public Patient getPatientByBedId(Long id) {
        //List aux = this.getJpaTemplate().find("from Patient p where p.bed in (select r.idRecurso from Recurso r where r.nr=?)", id);
        List aux = this.getJpaTemplate().find("from Patient p where p.bed.idRecurso=?)", id);
        if (aux != null && aux.size() == 1) {
            return (Patient) aux.get(0);
        }
        return null;
    ...
    public Patient getPatientByTag(String tag) {
        List aux = this.getJpaTemplate().find("from Patient p where p.patientTag.tag=?", tag);
        if (aux != null && aux.size() == 1) {
            return (Patient) aux.get(0);
        }
        return null;
    ...
    public List<Patient> findAll() {
        return this.getJpaTemplate().findByNamedQuery("Patient.findAll");
    }
     ...
     */
    @Transactional(readOnly = true)
    public List<Patient> loadAll(GridRequest_Extjs_v3 filters) {
        List<Patient> patient = this.find("from Patient p where p.exitus=0 and p.bed is not null " + filters.getQL("p", false), filters.getParamsValues(), filters.getStart(), filters.getLimit());
        return patient;
    }
    ...
    public Long loadSizeAll(GridRequest_Extjs_v3 filters) {
        List<Long> result = this.find("select count(*) from Patient p where p.exitus=0  and p.bed is not null" + filters.getQL("p", false), filters.getParamsValues());
        return result.get(0);
    }
}
