 ...
 */
package com.abada.contramed.persistence.dao.treatment.impl;
...
import com.abada.webcontramed.entities.GivesHistoricGroupType;
...
import com.abada.webcontramed.entities.ThreatmentInfoStatus;
...
import com.abada.webcontramed.utils.RangeItem;
...
public class ThreatmentDaoImpl extends JpaDaoSupport implements ThreatmentDao {
...
// <editor-fold defaultstate="collapsed" desc="Devuelve los tratamientos">
    ...
    private List<ThreatmentInfo> getActiveThreatment(Long idPatient, Date startTime, Date endTime) throws WebContramedException {
        ...
        List<OrderTiming> ordersTiming = this.getJpaTemplate().find(sql.toString(), idPatient);
    ...
    private List<GivesHistoric> getGivesHistoricInt(Long idOrderTiming, Date giveDate) {
        List<GivesHistoric> result = this.getJpaTemplate().find("from GivesHistoric gh where gh.orderTimingIdorderTiming.idorderTiming = ? and gh.orderTimingDate = ? order by gh.doseIddose.iddose, gh.eventDate desc", idOrderTiming, giveDate);//threatmentInfo.getIdOrderTiming(), threatmentInfo.getGiveTime());
        return result;
    }

    @Transactional(readOnly = true)
    private List<PrepareHistoric> getPrepareHistoricInt(Long idOrderTiming, Date giveDate) {
        return this.getJpaTemplate().find("from PrepareHistoric ph where ph.orderTimingIdorderTiming.idorderTiming = ? and ph.orderTimingDate = ? order by ph.doseIddose.iddose, ph.eventDate desc", idOrderTiming, giveDate);
    }
