...
public class CustomPageSizeImpl implements CustomPageSize {
    ...
    public Map<String, String> getSizePages() {
        ...
        for (int i = 0; i < declaredFields.length - 1; i++) {
            if (Modifier.isFinal(PageSize.class.getDeclaredFields()[i].getModifiers())) {
                if (Modifier.isStatic(PageSize.class.getDeclaredFields()[i].getModifiers())) {
                    ...
                    width = com.abada.trazability.jasper.commons.Utils.pixelsToMeasureUnit((int) r.getWidth(), MeasureUnit.CM);
                    height = com.abada.trazability.jasper.commons.Utils.pixelsToMeasureUnit((int) r.getHeight(), MeasureUnit.CM);
        ...
        for (Enumeration e = prop.keys(); e.hasMoreElements();) {
            ...
            int pWidth= com.abada.trazability.jasper.commons.Utils.measureUnitToPixels((Double.valueOf(aux[0]).doubleValue()), MeasureUnit.MM);
            int pHeight= com.abada.trazability.jasper.commons.Utils.measureUnitToPixels((Double.valueOf(aux[1]).doubleValue()), MeasureUnit.MM);
